...
namespace ScriptTrainer
{
    public class XmGUI : MonoBehaviour
    {
        ...
        public static bool Button(string text, int width, int height)
        {
            ...
            //guistyle.margin = new RectOffset(0, 0, 35 - height, 0);
            // 居中对齐
        ...
        public static void PaginationList(int page, int count, int limit, PaginationDo PaginationDo)
        {
            ...
            {
                //Debug.LogWarning(String.Format($"page:{page};count:{count};limit:{limit}"));
        ...
        public static void Switch(bool Switch, myFun onfun, myFun offfun)
        {
            Texture2D textureNormal = new Texture2D(1, 1, TextureFormat.RGBA32, false);
            textureNormal.SetPixel(0, 0, new Color32(30, 144, 255, 255));
            textureNormal.Apply();

            Texture2D textureActive = new Texture2D(1, 1, TextureFormat.RGBA32, false);
            textureActive.SetPixel(0, 0, new Color32(116, 125, 140, 255));  // rgba(112, 161, 255,1.0)
            textureActive.Apply();
            ...
            if (Switch)
            {
                if (GUILayout.Button("开", onStyle))
                {
                    onfun();
                }
                if (GUILayout.Button("关", offStyle))
                {
                    offfun();
                }
            }
            ...
            {
                if (GUILayout.Button("开", offStyle))
                {
                    onfun();
                }
                if (GUILayout.Button("关", onStyle))
                {
                    offfun();
                }
            }
